...
public class DestruirDistritoCondotiero extends Behaviour {
	...
	public void action() {
		...
		if(msg!=null){
			...
			try {
				contenido = (DestruirDistrito) myAgent.getContentManager().extractContent(msg);
			...
			if(ep.tieneDistrito(jObjetivo, distrito)){
				...
				if(distrito.getCoste()-1 <= datosJugadorActual.getDinero()){
					/* El jugador objetivo tiene el distrito y el jugador actual tiene dinero para pagar
					 * la destrucción */
